...
public class QuadTree {
    ...
    public QuadTree(int width, int height) {
        this.root_ = new Node(0, 0, width, height, null);
        width_ = width;
        height_ = height;
    }
    ...
    public void set(Point point) {
        set(point.getX(), point.getY(), point.getValue());
    }
    ...
    public Object remove(Point point) {
    ...
    public Point[] searchWithin(Node node) {
        ...
        this.navigate(node,
                new Func() {
                    @Override
                    public void call(QuadTree quadTree, Node n) {
                        arr.add(n.getPoint());
                    }
                });
    ...
    public void navigate(Node node, Func func) {
        ...
        switch (node.getNodeType()) {
            ...
            case POINTER:
                if (node.getNe() != null)
                    this.navigate(node.getNe(), func);
                if (node.getSe() != null)
                    this.navigate(node.getSe(), func);
                if (node.getSw() != null)
                    this.navigate(node.getSw(), func);
                if (node.getNw() != null)
                    this.navigate(node.getNw(), func);
                break;
        }
    }
    ...
    public Node find(Point point) {
        return find(root_, point.getX(), point.getY());
    }
